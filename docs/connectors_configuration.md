...
- *wait.time.registration.queue.consumer.in.ms* : wait time for a queue consumer (Source Connector) registration.
- *poll.delay.registration.queue.consumer.in.ms* : poll delay, i.e, wait time before start polling a registered consumer.
- *poll.interval.registration.queue.consumer.in.ms* : poll interval, i.e, time between every poll for a registered consumer.
